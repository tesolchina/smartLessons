...
class ContentLoader {
    ...
    async loadContent(url) {
        ...
        try {
            ...
            // Remove default welcome styling but keep container structure
            contentArea.className = 'bg-white dark:bg-gray-800 rounded-lg p-6 min-h-60 shadow-sm';
            
            // Add a back button at the top
            const backButton = document.createElement('div');
            backButton.className = 'mb-4 text-right';
            backButton.innerHTML = '<button onclick="resetContent()" class="text-sm text-gray-600 dark:text-gray-400 hover:text-primary underline">← Back to Overview</button>';
            ...
            // Extract content from the main container in the loaded HTML
            const bodyContent = tempDiv.querySelector('body');
            if (bodyContent) {
                // Look for the main content container (max-w-6xl div)
                const mainContainer = bodyContent.querySelector('.max-w-6xl');
                if (mainContainer) {
                    // Create a content wrapper div
                    const contentWrapper = document.createElement('div');
                    contentWrapper.className = 'w-full';
                    
                    // Clone and move the inner content
                    while (mainContainer.firstChild) {
                        contentWrapper.appendChild(mainContainer.firstChild);
                    }
                    
                    contentArea.appendChild(contentWrapper);
                } else {
                    // Fallback: move all body content
                    console.log('⚠️ No main container found, using fallback');
        ...
        setTimeout(() => {
            ...
            // Restore default styling for error content
            const contentArea = document.getElementById('contentArea');
            contentArea.className = 'bg-white dark:bg-gray-800 rounded-lg p-6 min-h-60 shadow-sm text-center text-gray-400 dark:text-gray-500';
            
            // Provide fallback content
            const fallbackContent = this.createFallbackContent(section);
            contentArea.innerHTML = fallbackContent;
    ...
    // Function to reset content area to welcome state
    resetContentArea() {
        const contentArea = document.getElementById('contentArea');
        contentArea.className = 'bg-white dark:bg-gray-800 rounded-lg p-6 min-h-60 shadow-sm text-center text-gray-400 dark:text-gray-500';
        contentArea.innerHTML = `
            <div class="text-5xl mb-3">🏛️</div>
            <h3 class="text-lg font-semibold mb-2">Welcome to Week 1!</h3>
            <p>Start your policy advocacy journey by selecting an activity above.</p>
            <div class="text-xs text-gray-300 mt-4">
                This week focuses on understanding Hong Kong's policy landscape and discovering issues that matter to you.
            </div>
        `;
    }
